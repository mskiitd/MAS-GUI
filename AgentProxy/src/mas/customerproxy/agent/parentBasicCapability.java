...
import mas.customerproxy.goal.LookUpAgentsCustomerGoal;
import mas.customerproxy.goal.RegisterAgentToBlackboardGoal;
import mas.customerproxy.goal.RegisterCustomerServiceGoal;
...
import mas.customerproxy.plan.LookUpAgentsCustomerPlan;
...
import mas.customerproxy.plan.RegisterCustomerServicePlan;
import mas.customerproxy.plan.SendConfirmedOrderPlan;
import mas.customerproxy.plan.SubscribeToZonesPlan;
...
public class parentBasicCapability extends Capability {
	...
	public static Set<Belief<?>> getBeliefs() {
		...
		Belief<AID> gsaAgent = new TransientBelief<AID>(ID.Customer.BeliefBaseConst.gsAgent);
		...
		gsaAgent.setValue(null);
		
		beliefs.add(gsaAgent);
		beliefs.add(bboard);
	...
	public static Set<Plan> getPlans() {
		...
		plans.add(new SimplePlan(RegisterAgentToBlackboardGoal.class, RegisterCustomerAgentToBlackboardPlan.class));
		
		plans.add(new SimplePlan(dispatchJobGoal.class,DispatchJobPlan.class));

		plans.add(new SimplePlan(CustomerSendNegotiationJobGoal.class, CustomerSendNegotiationJobPlan.class));
		...
		plans.add(new SimplePlan(LookUpAgentsCustomerGoal.class, LookUpAgentsCustomerPlan.class));
		
		plans.add(new SimplePlan(SubscribeToZonesGoal.class, SubscribeToZonesPlan.class));
		
		plans.add(new SimplePlan(RegisterCustomerServiceGoal.class, RegisterCustomerServicePlan.class));
	...
	protected void setup() {
		myAgent.addGoal(new RegisterCustomerServiceGoal());
		myAgent.addGoal(new RegisterAgentToBlackboardGoal());
		myAgent.addGoal(new LookUpAgentsCustomerGoal());
	}
}
